...
#show: nifty-ntnu-thesis.with(
  ...
  abstracts: (
    ([Abstract], [The `nifty-ntnu-thesis` template is a typst port of the `ntnuthesis` LaTeX class. It can be used for theses at all levels –
    bachelor, master, and PhD – and is available in English (British and
    American) and Norwegian (Bokmål and Nynorsk). This document is meant to
    serve (i) as a description of the document class, (ii) as an example of
    how to use it, and (iii) as a thesis template.]),
    ([Sammendrag], [Malen `nifty-ntnu-thesis` er en typst-oversettelse av LaTeX-klassen `ntnuthesis`. Den er tilrettelagt for avhandlinger på alle nivåer –
...
```typst #show: nifty-ntnu-thesis.with()``` at the beginning of your `.typ` file. Currently, it only supports English. The `nifty-ntnu-thesis` function has a number of options you can set, most of which will be described in this document. The rest will be documented in this template’s repository. 

The title page at the beginning of this document is a placeholder to be used when writing
the thesis. This should be removed before handing in the thesis by setting `titlepage: false`.
Instead, the official NTNU title page for the corresponding thesis type
...
<title-author-and-date>
The title of your thesis should be set by changing the `title` parameter of the template. The title will appear on the title page as well as in the running header of the even-numbered pages. If the title is too long for the header, you can use `short-title` to set a version for the header.
...
== Page Layout
<page-layout>
The document class is designed to work with two-sided printing. This
...
the same on even and odd pages in order to avoid the page
jumping back and forth while reading. 
...
#link(<fig:mapNTNU>)[2.1]. By default, figures are placed in the flow, exactly where they were specified. To change this, set the ```placement``` option to either `top`, `bottom`, or `auto`. To add an image, use ```typst #image()``` and set the `height` or `width` to include the graphics file. If the caption consists of a single sentence fragment (incomplete sentence), it should not be punctuated.
...
For figures composed of several sub-figures, the `subpar` module has been used. To use it, use the function ```typst #subfigure()```. See #link(<fig:subfig>)[2.2]
...
== Listings
<listings>
Code listings are also wrapped in a ```typst #figure()```. Code listings are defined by using three ``` `backticks` ```. The programming language can also be provided. See the typst documentation for details. The
...
== Fonts
<fonts>
Charter at 11pt has been selected as the main font for the thesis template. For code examples, the monospaced font should be used – for this, a scaled
version of DejaVu Sans Mono to match the main font is preselected. 
...
== Bibliography
<bibliography>
The bibliography is typeset as in standard typst. It is added in the initializing function as follows: ```typst bibliography: bibliography("thesis.bib")```.
With this setup, using `@` will give a number
only~@landes1951scrutiny, and ```typst #cite(, form: "prose") ``` will give the author and number like this: #cite(<landes1951scrutiny>, form: "prose");.
...
elements should be included, depends on several factors such as the
...
matter is by far the most important, and also the one that varies
...
#cite(<landes1951scrutiny>, form: "prose") has given some very nice
instructions on how to write a good abstract. A thesis from a Norwegian
University should contain abstracts in both Norwegian and English
irrespective of the thesis language (typically with the thesis
...
A table of contents should always be present in a document the size
...
/ Other lists\:: #block[
If there are other lists you would like to include, this would be a good
...
that do not fit within the body matter. This could be information about
...
the Arabic page numbering with page~1. There is great diversity in the
...
development report with more or less the following chapters:
...
requirements will appear throughout the project as, e.g., the user stories
...
contain the system architecture (client-server, cloud, databases,
...
to place this chapter somewhere else in the thesis.
]
...
/ Discussion\:: #block[
Here you should discuss all aspects of your thesis and project. How did
...
differently if you were to undertake the same project again, both
...
published material that you have used throughout the work on your
...
subject to change at any point in time. Such links, if necessary, should
...
task is to gain new knowledge about something. A thesis describing such
a project is typically structured as an extended form of a scientific
...
same and/or related topics. This should be described as background to
...
/ Background\:: #block[
Whereas background chapters are not necessarily needed in smaller
works, they are almost always needed in PhD theses. They may even be split
...
to it, should be presented here. The conclusion should answer
the research questions set out in the main introduction. See also
...
submission of the thesis, and thus have been through real quality
...
verbatim as a PDF. The publisher's PDF should be used if the copyright
permits it. This should be checked with the SHERPA/RoMEO
database#footnote[#link("http://sherpa.ac.uk/romeo/index.php");] or with
the publisher. Even when there is no general permission by the publisher,
